...
import subprocess, os, time, threading, json, re, sys, queue
...
def _auto_backup(wf):
    try: _save(BACKUP_FILE, wf)
    except Exception: pass

def get_cipher(password: str):
    if not CRYPTO_OK or not password: return None
    kdf = PBKDF2HMAC(
        algorithm=hashes.SHA256(),
        length=32,
        salt=b"elvar_salt_123",
        iterations=100000,
    )
    key = base64.urlsafe_b64encode(kdf.derive(password.encode()))
    return Fernet(key)

def read_file_content(fp, is_protected=False, password=None):
    if not os.path.exists(fp): return ""
    with open(fp, "rb" if is_protected else "r", encoding=None if is_protected else "utf-8") as f:
        content = f.read()
    if is_protected and password and CRYPTO_OK:
        try:
            cipher = get_cipher(password)
            return cipher.decrypt(content).decode("utf-8")
        except Exception:
            return "" # Decryption failed
    return content if isinstance(content, str) else content.decode("utf-8", errors="ignore")

def write_file_content(fp, content, is_protected=False, password=None):
    if is_protected and password and CRYPTO_OK:
        cipher = get_cipher(password)
...
BROWSER_PATHS = {
    "Opera GX":       [os.path.expandvars(r"%LOCALAPPDATA%\Programs\Opera GX\opera.exe"),
...
INCOGNITO_FLAGS = {
    "Opera GX":"--private","Google Chrome":"--incognito","Microsoft Edge":"--inprivate",
    "Brave":"--incognito","Firefox":"-private-window","Vivaldi":"--incognito","System Default":"",
}
NEW_WINDOW_FLAGS = {
    "Opera GX":"--new-window","Google Chrome":"--new-window","Microsoft Edge":"--new-window",
    "Brave":"--new-window","Firefox":"-new-window","Vivaldi":"--new-window","System Default":"",
}
CHROME_DEBUG_PORTS = {"Google Chrome":9222,"Microsoft Edge":9223,"Brave":9224,"Opera GX":9225,"Vivaldi":9226}
BROWSER_HISTORY_PATHS = {
    ...
    "Opera GX":       os.path.expandvars(r"%APPDATA%\Opera Software\Opera GX Stable\History"),
}
...
def auto_detect():
    ...
    for name in BROWSER_PATHS:
        p = detect_browser(name)
        if p is not None: r[name] = p
    return r
...
def play_sound():
    ...
    try: _ws.MessageBeep(_ws.MB_ICONASTERISK)
    except Exception: pass
...
def make_tray_icon():
    ...
    if getattr(sys, 'frozen', False):
        icon_path = os.path.join(sys._MEIPASS, "elvar_icon.ico")
    else:
        icon_path = "elvar_icon.ico"
    ...
    if not os.path.exists("elvar_icon.ico"):
        try:
            img.save("elvar_icon.ico", format="ICO", sizes=[(64, 64)])
        except Exception: pass
...
class NewWorkflowDialog(ctk.CTkToplevel):
    ...
    def _create(self):
        ...
        write_file_content(fp, "\n".join(urls) + "\n", flags_data.get("is_protected", False), self.master._settings.get("protected_password") if hasattr(self.master, "_settings") else None)
...
class WorkflowCard(ctk.CTkFrame):
    def __init__(self, parent, name, data, callbacks, is_protected_view=False, password=None, batch_var=None, **kw):
        ...
        if pinned and not is_protected_view:
            ctk.CTkLabel(top, text="📌", font=("Helvetica", 16), text_color="#FF9500").pack(side="left", padx=(0, 8))
            
        if is_protected_view:
            ctk.CTkLabel(top, text="🔒", font=("Helvetica", 16), text_color="#FFCC00").pack(side="left", padx=(0, 8))
        ...
        if is_protected_view:
            buttons = [
                ...
                ("👁", "toggle_hidden", "#3B82F6", "Toggle Hidden"),
                ("🔗", "ping", "#F43F5E", "Check Links"),
                ("🔓", "unprotect", "#EAB308", "Remove Protection"),
        ...
        if data.get("is_task_list"):
            buttons.insert(1, ("📋", "run", "#10B981", "Open Task List"))
        if data.get("is_queue"):
        ...
        def do_GET(self):
            if self.path == '/workflows':
                try:
                    ...
                    self.end_headers()
                    self.wfile.write(json.dumps({"workflows": wf_list}).encode('utf-8'))
                except Exception:
        ...
        def do_POST(self):
            try:
                ...
                if action == "save_session":
                    ...
                    queue.put({"kind": "ext_launch", "name": data.get("name"), "incognito": data.get("incognito", False), "new_window": data.get("new_window", False), "specific_urls": data.get("specific_urls")})
                    ...
                    queue.put({"kind": "ext_get_protected", "name": data.get("name"), "password": data.get("password"), "event": event, "result": result})
                    event.wait(timeout=5.0)
                    
                self.send_response(200 if result.get("status") == "ok" else 500)
                self.send_header('Access-Control-Allow-Origin', '*')
                self.send_header('Content-Type', 'application/json')
                self.end_headers()
...
class App(ctk.CTk):
    def __init__(self):
        ...
        threading.Thread(target=start_ext_server, args=(self._msg_queue,), daemon=True).start()
        self._build()
        ...
        try:
            while True:
                ...
                if kind == "ask":
                    ...
                    try:
                        expected = self._settings.get("protected_password")
                        if expected and pwd == expected:
                            wf_data = self._workflows.get(name)
                            if wf_data:
                                fp = wf_data.get("path", "")
                                if fp and os.path.exists(fp):
                                    content = read_file_content(fp, True, expected)
                        ...
                        if urls:
                            if target == "_new_session" and hasattr(self, "_save_session_dlg") and self._save_session_dlg and self._save_session_dlg.winfo_exists():
                                ...
                                if target != "_new_workflow" and target != "_new_session" and target in self._workflows:
                                    ...
                                    fp = wf_data.get("path", "")
                                    if fp:
                                        content = read_file_content(fp, wf_data.get("is_protected", False), self._settings.get("protected_password"))
                                        new_content = content + "\n" + "\n".join(urls) + "\n"
                                        write_file_content(fp, new_content, wf_data.get("is_protected", False), self._settings.get("protected_password"))
                                    ...
                                    is_protected = data.get("is_protected", False)
                                    password = self._settings.get("protected_password")
    ...
    def _build(self):
        ...
        try:
            from PIL import Image
            if getattr(sys, 'frozen', False):
                img_path = os.path.join(sys._MEIPASS, "elvar_icon.ico")
            else:
                img_path = "elvar_icon.ico"
        ...
        tab_defs = [("workflows", "⌘ Workflows"), ("launcher", "▶ Launcher"), ("sessions", "◉ Sessions"),
                    ("history", "⏱ History"), ("analytics", "📊 Analytics"), ("protected", "🔒 Protected")]
    ...
    def _build_workflows(self, parent):
        ...
        self._wf_count_lbl = ctk.CTkLabel(hdr, text=f"  {len(self._workflows)} saved", font=("Helvetica", 14), text_color=("#8E8E93", "#98989D"))
        self._wf_count_lbl.pack(side="left", padx=10)
        ...
        self._batch_btn = ctk.CTkButton(btn_row, text="Batch Mode", command=self._toggle_batch_mode, fg_color="transparent", border_width=1, text_color=("#000000", "#FFFFFF"), width=100)
        self._batch_btn.pack(side="left", padx=5)
        
        ctk.CTkButton(btn_row, text="+ New", command=self._new_workflow, fg_color="#007AFF", width=100).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="+ Queue", command=self._new_queue, fg_color="#34C759", hover_color="#28A745", width=100).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="Import .txt", command=self._import_file, fg_color="transparent", border_width=1, text_color=("#000000", "#FFFFFF"), width=100).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="Export All .zip", command=self._export_all_zip, fg_color="#5AC8FA", width=120).pack(side="left", padx=5)
        
        search_row = ctk.CTkFrame(parent, fg_color="transparent")
        search_row.pack(fill="x", padx=30, pady=(0, 10))
        
        self._wf_search = ctk.CTkEntry(search_row, placeholder_text="⌕ Search workflows...", font=("Helvetica", 14), height=35)
        self._wf_search.pack(side="left", fill="x", expand=True)
        self._wf_search.bind("<KeyRelease>", lambda e: self._render_wf())
        
        self._sort_var = ctk.StringVar(value="default")
        self._show_pinned_first = ctk.BooleanVar(value=True)
        ...
        for label, val in [("Default", "default"), ("Name", "name"), ("Runs", "runs"), ("Last", "last_run"), ("Links", "links")]:
            ctk.CTkRadioButton(sort_frame, text=label, variable=self._sort_var, value=val, font=("Helvetica", 12),
                               command=self._render_wf).pack(side="left", padx=5)
                               
        ctk.CTkCheckBox(sort_frame, text="📌 first", variable=self._show_pinned_first, font=("Helvetica", 12),
                        command=self._render_wf).pack(side="left", padx=10)
                        
        self._wf_scroll = ctk.CTkScrollableFrame(parent, fg_color="transparent")
        self._wf_scroll.pack(fill="both", expand=True, padx=20, pady=(0, 20))
        
        self._batch_actions_frame = ctk.CTkFrame(parent, fg_color="transparent")
        
        ctk.CTkButton(self._batch_actions_frame, text="Select All", command=self._batch_select_all, fg_color="#8E8E93", hover_color="#636366", width=120).pack(side="left", padx=10)
        ctk.CTkButton(self._batch_actions_frame, text="Delete Selected", command=self._batch_delete, fg_color="#FF3B30", hover_color="#D32F2F", width=120).pack(side="right", padx=10)
        ctk.CTkButton(self._batch_actions_frame, text="Export Selected", command=self._batch_export, fg_color="#5AC8FA", hover_color="#34AADC", width=120).pack(side="right", padx=10)
    ...
    def _render_wf(self):
        ...
        for name, data in matches:
            ...
            card = WorkflowCard(self._wf_scroll, name, data, callbacks=cbs, password=self._settings.get("protected_password"), batch_var=batch_var)
    ...
    def _protect_workflow(self, name):
        if not self._settings.get("protected_password"):
        ...
        if os.path.exists(fp):
            ...
            write_file_content(fp, content, True, self._settings.get("protected_password"))
    ...
    def _unprotect_workflow(self, name):
        ...
        fp = data.get("path", "")
        if os.path.exists(fp):
            content = read_file_content(fp, True, self._settings.get("protected_password"))
    ...
    def _open_editor(self, name, skip_auth=False):
        ...
        LinkEditorDialog(self, name, data, on_save=on_save, password=self._settings.get("protected_password"))
    ...
    def _export_workflow_menu(self, name):
        ...
        if save_fp:
            try:
                content = read_file_content(fp, data.get("is_protected", False), self._settings.get("protected_password"))
            ...
            with zipfile.ZipFile(fp, "w", zipfile.ZIP_DEFLATED) as zf:
                ...
                for name, data in self._workflows.items():
                    ...
                    if wfp and os.path.isfile(wfp):
                        content = read_file_content(wfp, data.get("is_protected", False) if isinstance(data, dict) else False, self._settings.get("protected_password"))
    ...
    def _run_workflow(self, name, skip_auth=False, ext_incognito=None, ext_new_window=None):
        ...
        if data.get("is_queue"):
            RunQueueDialog(self, name, data, on_run=lambda n, urls, d: self._run_queue_selected(n, urls, d, ext_incognito, ext_new_window), password=self._settings.get("protected_password"))
            return
    ...
    def _build_settings(self, parent):
        ...
        ctk.CTkSwitch(sec1, text="Dark Mode", variable=self.theme_var, onvalue="Dark", offvalue="Light", command=self._toggle_theme, font=("Helvetica", 14)).pack(anchor="w", padx=20, pady=10)
        ctk.CTkSwitch(sec1, text="Play sound on completion", variable=self.sound_var, font=("Helvetica", 14)).pack(anchor="w", padx=20, pady=10)
        ctk.CTkSwitch(sec1, text="Strip tracking parameters (UTM tags) from URLs", variable=self.strip_utm_var, font=("Helvetica", 14)).pack(anchor="w", padx=20, pady=10)
        ctk.CTkSwitch(sec1, text="Auto-Pin Tabs (where supported)", variable=self.auto_pin_var, font=("Helvetica", 14)).pack(anchor="w", padx=20, pady=10)
        ctk.CTkSwitch(sec1, text="Auto-Lock Protected Area on minimize/tray", variable=self.auto_lock_var, font=("Helvetica", 14)).pack(anchor="w", padx=20, pady=10)
        ctk.CTkSwitch(sec1, text="Disable Activity Log entirely", variable=self.disable_log_var, font=("Helvetica", 14)).pack(anchor="w", padx=20, pady=10)
        ctk.CTkSwitch(sec1, text="Dry Run mode (simulate opening tabs)", variable=self.dry_run, font=("Helvetica", 14)).pack(anchor="w", padx=20, pady=(10, 20))
        ...
        ce = ctk.CTkEntry(sec2, textvariable=self.custom_exe, font=("Courier New", 14), height=35)
        ce.pack(fill="x", padx=20, pady=10)
        ctk.CTkButton(sec2, text="Browse...", command=lambda: self.custom_exe.set(filedialog.askopenfilename() or self.custom_exe.get()), fg_color="transparent", border_width=1, text_color=("#000000", "#FFFFFF")).pack(anchor="w", padx=20, pady=(0, 20))
        ...
        ctk.CTkButton(ext_row, text="Save Extension", command=self._save_extension_files, fg_color="#007AFF", width=120).pack(side="right")
        ...
        ctk.CTkButton(bk_row, text="⭳ Backup", command=self._backup_data, fg_color="#007AFF", width=100).pack(side="right")
        ...
        ctk.CTkButton(rs_row, text="⭱ Restore", command=self._restore_data, fg_color="transparent", border_width=1, text_color=("#000000", "#FFFFFF"), width=100).pack(side="right")
        ...
        if GDRIVE_OK:
            ...
            ctk.CTkButton(gd_bk_row, text="⭳ Backup to Drive", command=self._backup_gdrive, fg_color="#34A853", hover_color="#2B8A44", width=140).pack(side="right")
            ...
            ctk.CTkButton(gd_rs_row, text="⭱ Restore from Drive", command=self._restore_gdrive, fg_color="#4285F4", hover_color="#3367D6", width=140).pack(side="right")
        else:
        ...
        ctk.CTkButton(pw_row, text="Change Password", command=self._change_password, fg_color="transparent", border_width=1, text_color=("#000000", "#FFFFFF"), width=120).pack(side="right")
        ...
        ctk.CTkButton(frp_row, text="Force Reset", command=self._force_reset_password, fg_color="transparent", border_width=1, text_color=("#FF3B30", "#FF453A"), border_color=("#FF3B30", "#FF453A"), hover_color=("#FF3B30", "#FF453A"), width=120).pack(side="right")
        ...
        ctk.CTkButton(rs_all_row, text="⚠ Reset", command=self._reset_all_data, fg_color="transparent", border_width=1, border_color="#FF3B30", text_color="#FF3B30", width=100).pack(side="right")
        
        ctk.CTkLabel(sec3, text="DATA FOLDER", font=("Helvetica", 12, "bold"), text_color=("#8E8E93", "#98989D")).pack(anchor="w", padx=20, pady=(15, 5))
        ctk.CTkLabel(sec3, text=APP_DIR, font=("Courier New", 14)).pack(anchor="w", padx=20, pady=5)
        ctk.CTkButton(sec3, text="Open Folder", command=lambda: open_folder(APP_DIR), fg_color="transparent", border_width=1, text_color=("#000000", "#FFFFFF")).pack(anchor="w", padx=20, pady=(5, 20))
    ...
    def _save_extension_files(self):
        ...
        try:
            ...
            with open(os.path.join(ext_dir, "manifest.json"), "w", encoding="utf-8") as f:
                f.write("""{
  "manifest_version": 3,
  "name": "Elvar Companion",
  "version": "1.0",
  "description": "Send open tabs directly to Elvar.",
  "permissions": ["tabs", "tabGroups", "contextMenus"],
  "host_permissions": ["http://127.0.0.1:31337/"],
  "icons": {
    "128": "icon.png"
  },
  "background": {
...
<head>
  <style>
    ...
    .header h2 { margin: 0; font-size: 16px; font-weight: 600; letter-spacing: -0.01em; }
    
    .container { padding: 16px; }
    
    h3 { margin-top: 0; font-size: 14px; font-weight: 600; margin-bottom: 12px; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.05em; }
    ...
    .msg { display: none; margin-top: 12px; text-align: center; font-size: 13px; font-weight: 500; padding: 8px; border-radius: 6px; animation: slideIn 0.3s ease; }
    .success { background: rgba(52, 199, 89, 0.1); color: #30D158; border: 1px solid rgba(52, 199, 89, 0.2); }
    .error { background: rgba(255, 69, 58, 0.1); color: #FF453A; border: 1px solid rgba(255, 69, 58, 0.2); }
    ...
    .tab-list::-webkit-scrollbar-thumb { background: #444; border-radius: 3px; }
    ...
    .tab-item:hover { background: var(--surface-hover); }
    .tab-item input[type="checkbox"] { margin-right: 12px; accent-color: var(--primary); width: 16px; height: 16px; cursor: pointer; }
    .tab-item span { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    
    .section { margin-bottom: 24px; animation: fadeIn 0.4s ease; }
    @keyframes fadeIn {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    ...
    .workflows::-webkit-scrollbar { width: 6px; }
    .workflows::-webkit-scrollbar-thumb { background: #444; border-radius: 3px; }
    ...
    .wf-btn:hover { background: var(--surface-hover); border-color: #555; transform: none; }
    .wf-btn::after { content: '▶'; font-size: 10px; color: var(--primary); opacity: 0; transition: opacity 0.2s; }
    .wf-btn:hover::after { opacity: 1; }
    ...
    .tab-btn:hover { background: transparent; color: var(--text); transform: none; }
...
<body>
  <div class="header">
    <img src="icon.png" alt="Elvar">
    <h2>Elvar Companion</h2>
  </div>
  
  <div class="tabs-header">
    <button class="tab-btn active" id="tabSave">Save Tabs</button>
    <button class="tab-btn" id="tabLaunch">Launch</button>
  </div>

  <div class="container">
    <div id="contentSave" class="tab-content active">
      <input type="text" id="tabSearch" placeholder="Search tabs..." style="margin-bottom: 8px;">
      <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
        <button id="selectAllTabs" style="width: 48%; padding: 6px; font-size: 12px; background: var(--surface); border: 1px solid var(--border);">Select All</button>
        <button id="deselectAllTabs" style="width: 48%; padding: 6px; font-size: 12px; background: var(--surface); border: 1px solid var(--border);">Deselect All</button>
      </div>
      <div class="tab-list" id="tabList"></div>
      <div class="toggle-row">
        <span class="toggle-label" id="saveTypeLabel">Save as Session</span>
        <label class="switch">
          <input type="checkbox" id="saveTypeToggle">
          <span class="slider"></span>
        </label>
      </div>
      <div class="toggle-row" id="protectedToggleRow" style="display: none;">
        <span class="toggle-label">Save as Protected</span>
        <label class="switch">
          <input type="checkbox" id="isProtectedToggle">
          <span class="slider"></span>
        </label>
      </div>
      <select id="targetSelect" style="display: none;">
        <option value="_new_workflow">Create New Workflow</option>
      </select>
      ...
      <button id="saveCloseBtn" class="danger">
      ...
      <button id="closeBtn" class="danger" style="margin-top: 8px;">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        Close Selected Tabs
      </button>
    </div>
    
    <div id="contentLaunch" class="tab-content">
      <div class="toggle-row">
        <span class="toggle-label">Incognito Mode</span>
        <label class="switch">
          <input type="checkbox" id="extIncognito">
          <span class="slider"></span>
        </label>
      </div>
      <div class="toggle-row">
        <span class="toggle-label">Force New Window</span>
        <label class="switch">
          <input type="checkbox" id="extNewWindow">
          <span class="slider"></span>
        </label>
      </div>
      
      <h3 style="margin-top: 16px;">Workflows</h3>
      <div class="workflows" id="wfList">
        <p style="text-align:center; margin: 20px 0; color: var(--text-muted); font-size: 13px;">Loading workflows...</p>
      </div>
      
      <h3 style="margin-top: 16px;">Protected Workflows</h3>
      <div class="workflows" id="protectedWfList">
        <p style="text-align:center; margin: 20px 0; color: var(--text-muted); font-size: 13px;">Loading workflows...</p>
      </div>
    </div>
    ...
    if (selectedTabs.length > 0) {
      ...
      setTimeout(() => window.close(), 1000);
            ...
            with open(os.path.join(ext_dir, "background.js"), "w", encoding="utf-8") as f:
                f.write("""chrome.runtime.onInstalled.addListener(() => {
  ...
  if (info.menuItemId === "add-to-elvar") {
    ...
    try {
      ...
      console.error("Elvar not running", e);
            ...
            # Write README.txt
            with open(os.path.join(ext_dir, "README.txt"), "w", encoding="utf-8") as f:
                f.write("""Elvar Companion Extension Installation Guide
==========================================
...
5. Select this folder ("ElvarExtension").
6. The Elvar Companion extension is now installed!
7. Pin it to your toolbar for quick access.""")
    ...
    def _change_password(self):
        ...
        if expected:
            ...
            if not ans: return
            if ans != expected and ans.lower().strip() != self._settings.get("security_answer", ""):
        ...
        def save():
            ...
            if pwd:
                if not q_entry.get() or not a_entry.get():
                    ...
                    return
                self._settings["protected_password"] = pwd
            ...
            _save(SETTINGS_FILE, self._settings)
            Toast(self, "Password updated successfully." if pwd else "Password removed.", "green")
            dialog.destroy()
    ...
    def _build_launcher(self, parent):
        ...
        ctk.CTkEntry(f_row, textvariable=self.sel_file, state="disabled", font=("Helvetica", 14)).pack(side="left", fill="x", expand=True, padx=10)
        ctk.CTkButton(f_row, text="Browse", command=self._pick_file, width=100).pack(side="left")
        ...
        self.qs_var = ctk.StringVar(value="Select Workflow...")
        self.qs_dropdown = ctk.CTkOptionMenu(qs_row, variable=self.qs_var, values=["Select Workflow..."], font=("Helvetica", 14), dynamic_resizing=False)
        self.qs_dropdown.pack(side="left", fill="x", expand=True, padx=10)
        ctk.CTkButton(qs_row, text="Load", command=self._load_quick_select, width=100, fg_color="#5AC8FA", hover_color="#34AADC").pack(side="left")
        ...
        browsers = ["System Default"] + list(self._detected.keys()) + ["Custom..."]
        ctk.CTkComboBox(b_row, values=browsers, variable=self.sel_browser, font=("Helvetica", 14), state="readonly").pack(side="left", fill="x", expand=True, padx=10)
        ...
        ctk.CTkEntry(m_row, textvariable=self.range_start, width=60).pack(side="left", padx=5)
        ctk.CTkLabel(m_row, text="End:", font=("Helvetica", 14)).pack(side="left")
        ctk.CTkEntry(m_row, textvariable=self.range_end, width=60).pack(side="left", padx=5)
        ...
        ctk.CTkEntry(mt_row, textvariable=self.batch_size, width=60).pack(side="left", padx=10)
        ...
        ctk.CTkSwitch(opt_row, text="Incognito Mode", variable=self.incognito, font=("Helvetica", 14)).pack(side="left", padx=(0, 20))
        ctk.CTkSwitch(opt_row, text="Force New Window", variable=self.new_window, font=("Helvetica", 14)).pack(side="left")
        
        # Controls
        c_row = ctk.CTkFrame(body, fg_color="transparent")
        c_row.pack(fill="x", pady=20)
        self._run_btn = ctk.CTkButton(c_row, text="▶ Run", command=self._run, fg_color="#34C759", hover_color="#28A745", font=("Helvetica", 14, "bold"), height=36)
        self._run_btn.pack(side="left", fill="x", expand=True, padx=(0, 5))
        self._pause_btn = ctk.CTkButton(c_row, text="⏸ Pause", command=self._pause, fg_color="#FF9500", hover_color="#E68A00", font=("Helvetica", 14, "bold"), height=36, state="disabled")
        self._pause_btn.pack(side="left", fill="x", expand=True, padx=5)
        self._stop_btn = ctk.CTkButton(c_row, text="⏹ Stop", command=self._stop, fg_color="#FF3B30", hover_color="#D32F2F", font=("Helvetica", 14, "bold"), height=36, state="disabled")
        self._stop_btn.pack(side="left", fill="x", expand=True, padx=(5, 0))
        
        # Progress
        self._prog = ctk.CTkProgressBar(body)
        self._prog.pack(fill="x", pady=10)
        self._prog.set(0)
        
        # Log
        self._log_txt = ctk.CTkTextbox(body, font=("Courier New", 12), state="disabled")
        self._log_txt.pack(fill="both", expand=True, pady=10)
        ...
        try:
            ...
            if workflow_name and hasattr(self, '_workflows'):
                ...
                if isinstance(data, dict):
                    is_protected = data.get("is_protected", False)
            content = read_file_content(fp, is_protected, self._settings.get("protected_password"))
            if "{input}" in content:
    ...
    def _worker(self, fp, browser, wf_name, user_input=None, specific_urls=None, delete_after=None, ext_incognito=None, ext_new_window=None):
        try:
            ...
            if specific_urls:
                raw_links = specific_urls
                
            if user_input:
                raw_links = [l.replace("{input}", user_input) for l in raw_links]
            
            orig_links = [normalize_url(l) for l in raw_links if normalize_url(l)]
            links = [strip_tracking(u) for u in orig_links] if self.strip_utm_var.get() else orig_links
            
            pairs = list(zip(orig_links, links))
            
            mode = data.get("settings", {}).get("run_mode", self.run_mode.get())
            if mode == "reverse": pairs.reverse()
            elif mode == "shuffle": random.shuffle(pairs)
            
            start_idx = max(0, data.get("settings", {}).get("range_start", self.range_start.get()) - 1)
            end_val = data.get("settings", {}).get("range_end", self.range_end.get())
            end_idx = end_val if end_val > 0 else len(pairs)
            pairs = pairs[start_idx:end_idx]
            
            max_t = data.get("settings", {}).get("max_tabs", self.max_tabs.get())
            if max_t > 0: pairs = pairs[:max_t]
                ...
                if isinstance(data, dict):
                    ...
                    if should_delete and opened_orig_urls:
                        try:
                            ...
                            content = read_file_content(orig_fp, data.get("is_protected", False), self._settings.get("protected_password"))
                            ...
                            write_file_content(orig_fp, "".join(new_lines), data.get("is_protected", False), self._settings.get("protected_password"))
    ...
    def _build_sessions(self, parent):
        ...
        self._sess_count = ctk.CTkLabel(hdr, text=f"  {len(self._sessions)} saved", font=("Helvetica", 14), text_color=("#8E8E93", "#98989D"))
        self._sess_count.pack(side="left", padx=10)
        
        btn_row = ctk.CTkFrame(hdr, fg_color="transparent")
        btn_row.pack(side="right")
        ctk.CTkButton(btn_row, text="+ Save Session", command=self._save_session, fg_color="#AF52DE", hover_color="#8E33B7", width=120).pack(side="left")
        
        self._sess_scroll = ctk.CTkScrollableFrame(parent, fg_color="transparent")
        self._sess_scroll.pack(fill="both", expand=True, padx=20, pady=(0, 20))
        self._render_sessions()
    ...
    def _render_sessions(self):
        ...
        for name, data in sorted(self._sessions.items(), key=lambda x: x[1].get("date", ""), reverse=True):
            ...
            ctk.CTkButton(btn_row, text="✕", width=30, height=28, fg_color="transparent", text_color="#FF3B30", hover_color=("#E5E5EA", "#3A3A3C"),
    ...
    def _build_history(self, parent):
        ...
        btn_row = ctk.CTkFrame(hdr, fg_color="transparent")
        btn_row.pack(side="right")
        ctk.CTkButton(btn_row, text="Export CSV", command=self._export_history_csv, fg_color="#5AC8FA", width=120).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="Clear History", command=self._clear_history, fg_color="#FF3B30", hover_color="#D32F2F", width=120).pack(side="left")
        
        self._hist_scroll = ctk.CTkScrollableFrame(parent, fg_color="transparent")
        self._hist_scroll.pack(fill="both", expand=True, padx=20, pady=(0, 20))
    ...
    def _build_analytics(self, parent):
        ...
        btn_row = ctk.CTkFrame(hdr, fg_color="transparent")
        btn_row.pack(side="right")
        ctk.CTkButton(btn_row, text="Refresh", command=self._render_analytics, fg_color="#007AFF", width=100).pack(side="left")
        
        self._analytics_scroll = ctk.CTkScrollableFrame(parent, fg_color="transparent")
        self._analytics_scroll.pack(fill="both", expand=True, padx=20, pady=(0, 20))
    ...
    def _build_protected(self, parent):
        self._protected_parent = parent
        self._protected_auth = False
        
        self._prot_auth_frame = ctk.CTkFrame(parent, fg_color="transparent")
        self._prot_auth_frame.pack(fill="both", expand=True, padx=30, pady=20)
        
        box = ctk.CTkFrame(self._prot_auth_frame, fg_color=("#FFFFFF", "#1C1C1E"), corner_radius=15)
        box.place(relx=0.5, rely=0.5, anchor="center")
        
        ctk.CTkLabel(box, text="🔒 Protected Area", font=("Helvetica", 24, "bold")).pack(pady=(30, 10), padx=50)
        ...
        self._prot_pw_entry = ctk.CTkEntry(box, show="*", font=("Helvetica", 14), width=250, height=40, justify="center")
        self._prot_pw_entry.pack(pady=(0, 10))
        self._prot_pw_entry.bind("<Return>", lambda e: self._auth_protected())
        
        self._prot_err_lbl = ctk.CTkLabel(box, text="", font=("Helvetica", 12), text_color="#FF3B30")
        self._prot_err_lbl.pack(pady=(0, 10))
        
        btn_frame = ctk.CTkFrame(box, fg_color="transparent")
        btn_frame.pack(pady=(0, 30))
        
        ctk.CTkButton(btn_frame, text="Unlock", command=self._auth_protected, fg_color="#FFCC00", hover_color="#E6B800", font=("Helvetica", 14, "bold"), height=36, width=120).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Forgot?", command=self._forgot_password, fg_color="transparent", border_width=1, text_color=("#000000", "#FFFFFF"), height=36, width=120).pack(side="left", padx=5)
        
        self._prot_main_frame = ctk.CTkFrame(parent, fg_color="transparent")
        
        hdr = ctk.CTkFrame(self._prot_main_frame, fg_color="transparent")
        hdr.pack(fill="x", padx=30, pady=20)
        ctk.CTkLabel(hdr, text="Protected Workflows", font=("Helvetica", 28, "bold")).pack(side="left")
        self._prot_count_lbl = ctk.CTkLabel(hdr, text="  0 secured", font=("Helvetica", 14), text_color="#FFCC00")
        self._prot_count_lbl.pack(side="left", padx=10)
        
        btn_row = ctk.CTkFrame(hdr, fg_color="transparent")
        btn_row.pack(side="right")
        ctk.CTkButton(btn_row, text="🔒 Lock", command=self._lock_protected, fg_color="transparent", border_width=1, text_color=("#000000", "#FFFFFF"), width=100).pack(side="left")
        
        self._prot_scroll = ctk.CTkScrollableFrame(self._prot_main_frame, fg_color="transparent")
        self._prot_scroll.pack(fill="both", expand=True, padx=20, pady=(0, 20))
    ...
    def _forgot_password(self):
        ...
        if ans and ans.lower().strip() == self._settings.get("security_answer", ""):
            messagebox.showinfo("Password Recovery", f"Your password is:\n{self._settings.get('protected_password')}")
        elif ans:
    ...
    def _render_protected(self):
        ...
        for name, data in matches:
            card = WorkflowCard(self._prot_scroll, name, data, callbacks=cbs, is_protected_view=True, password=self._settings.get("protected_password"))
            card.pack(fill="x", pady=6, padx=10)
